...
export class GroupchatPage {
  ...
  constructor(
    ...
    public imgstore: ImagesProvider,
    public loadingCtrl: LoadingController
    ) {
    this.alignuid = firebase.auth().currentUser.uid;
    this.photoURL = firebase.auth().currentUser.photoURL;
    this.groupName = this.navParams.get('groupName');
    this.groupprov.getownership(this.groupName).then((res) => {
      if (res)
        this.owner = true;
    }).catch((err) => {
      alert(err);
    })
    this.groupprov.getgroupmsgs(this.groupName);
    this.events.subscribe('newgroupmsg', () => {
      this.allgroupmsgs = [];
      this.imgornot = [];
      this.allgroupmsgs = this.groupprov.groupmsgs;
  ...
  sendpicmsg() {
    ...
    this.imgstore.picmsgstore().then((imgurl) => {
      loader.dismiss();
      this.groupprov.addgroupmsg(imgurl).then(() => {
        this.scrollto();
        this.newmessage = '';
      })
    }).catch((err) => {
    ...
    let sheet = this.actionSheet.create({
      ...
      buttons: [
        ...
        },
        {
          ...
          icon: 'trash',
          handler: () => {
            this.groupprov.deletegroup().then(() => {
              this.navCtrl.pop();
            }).catch((err) => {
              console.log(err);
            })
      ...
      buttons: [
        {
          ...
          icon: 'log-out',
          handler: () => {
            this.groupprov.leavegroup().then(() => {
              this.navCtrl.pop();
            }).catch((err) => {
              console.log(err);
            })
  ...
  addgroupmsg() {
    this.groupprov.addgroupmsg(this.newmessage).then(() => {
      this.scrollto();
      this.newmessage = '';
    })
  }
